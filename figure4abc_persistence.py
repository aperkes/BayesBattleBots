...
for i in tqdm(range(iterations)):
    ...
    if np.max(tank2.sizes) != f.size:
        ...
        winner_record.append(tank2.win_record[f.idx,idx_bigger]/params.n_fights)
    else:
        idx_bigger = f.idx 
    biggers.append(tank.fishes[idx_bigger])
    if np.min(tank3.sizes) != f2.size:
        idx_smaller = np.arange(tank.n_fish)[ranks == (l_rank - 1)][0]
        loser_record.append(tank3.win_record[f2.idx,idx_bigger]/params.n_fights)
    else:
        idx_smaller = f2.idx
...
if False:
    ax3.bar(0,np.mean(winner_record),color='gold',yerr=np.std(winner_record)/np.sqrt(len(winners)))
    ax3.bar(1,np.mean(loser_record),color='darkblue',yerr=np.std(loser_record)/np.sqrt(len(losers)))
else:
    ax3.bar(0,np.mean(winner_record)-0.5,bottom=0.5,color='gold',yerr=np.std(winner_record)/np.sqrt(len(winners)))
    ax3.bar(1,np.mean(loser_record)-0.5,bottom=0.5,color='darkblue',yerr=np.std(loser_record)/np.sqrt(len(losers)))

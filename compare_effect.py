...
win_prop = win_count1 / iterations
loss_prop = win_count2 / iterations
win_record = np.zeros(iterations)
loss_record = np.zeros(iterations)
win_record[:win_count1] = 1
loss_record[:win_count2] = 1
win_sem = np.std(win_record) / np.sqrt(iterations)
loss_sem = np.std(loss_record) / np.sqrt(iterations)
...
if False:
    ...
    ax.bar([1,2],[win_prop-.5,loss_prop-.5],bottom=.5,color=['gold','darkblue'])
    ax.set_xticks([1,2])
    ax.set_xticklabels(['win','loss'])
    ax.axhline(.5,linestyle=':',color='black')
else:
    
    ax.bar([1,2],[win_prop-.5,loss_prop-.5],bottom=.5,color=['gold','darkblue'],yerr=[win_sem,loss_sem])
    ax.set_xticks([1,2])
    ax.set_xticklabels(['win','loss'])
    ax.axhline(.5,linestyle=':',color='black')
    ...
    ax.set_ylabel('Proportion won \nfollowing staged event')
#fig.show()
